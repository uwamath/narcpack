...
class Rational_approximation():
	...
	def __init__(self, coeffs=None, interval=[-1,1]):
		"""
		coeffs should either be
			(1) a dict with keys num and denom with
			corresponding values the Chebyshev coefficients of the numerator
			and denominator; or
			(2) a list-like object whose first and second entries are lists of
			the Chebyshev coefficients of the numerator and denominator
		"""
		if coeffs is None:
			...
			if isinstance(coeffs, dict):
				...
				self.coeffs = {'num':C.Chebyshev(coeffs[0]), 'denom':C.Chebyshev(coeffs[1])}
	...
	def fit(self, f, interval=[-1,1], degs=[3,3]):
		"""
		...
		interval : array_like, optional (not quite working yet--best to leave as [-1,1])
		...
		Returns
		----------
		g : Function from the reals to the reals
		    The rational function approximation to f.
		    Its numerator and denominator, p and q, are Chebyshev polynomials of degree 
		    degs[0] and degs[1], respectively.
		...
		self.coeffs['num'] = C.Chebyshev(x[:n+1])
		self.coeffs['denom'] = C.Chebyshev(np.concatenate(([1],x[n+1:]),axis=None))
